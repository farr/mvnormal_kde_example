...
def conditional_normal_model(log_p_other, samples, prior_wts, n_normal, *args, samples_sel=None, pdraw_sel=None, n_draw_sel=None, predictive=False, **kwargs):
    r"""
    ...
    vector of these parameters after marginalizing out the Gaussian dimensions.
    Unlike :func:`marginal_normal_model`, this model makes no assumptions about
    the width / scale of the likelihood function once the Gaussian parameters
    have been marginalized out.  If, instead, the marginal likelihood is much
    narrower than the population over the non-Gaussian parameters, then
    :func:`marginal_normal_model` will be more efficient, and should be used.
    ...
    * `R`: if `samples_sel` are given, then this is a draw from the "rate"
    ...
    sigma_cond = np.linalg.inv(np.linalg.inv(sigma)[:, :n_normal, :n_normal])
    kde_bw = sigma_cond / nsamp**(2/(4+n_normal))
    ...
    marginal_kde_bw = kde_bw + jnp.diag(jnp.square(sigmas))

    logp_normal = dist.MultivariateNormal(mu, marginal_kde_bw[:, jnp.newaxis, :, :]).log_prob(samples[..., :n_normal])
    logp_other_0 = log_p_other(samples[..., n_normal:])
    ...
    if predictive:
        ...
        a, A, _ = gaussian_marginalization(samples[..., :n_normal], kde_bw, mu, sigmas)
        a_i = a[np.arange(nobs), inds, :]
        A_i = A[np.arange(nobs), inds, :, :]
        theta_g = numpyro.sample('theta_gaussian', dist.MultivariateNormal(a_i, A_i))
        theta = numpyro.deterministic('theta', jnp.concatenate((theta_g[:, :n_normal], jnp.array(samples)[np.arange(nobs), inds, n_normal:]), axis=1))
...
def marginal_normal_model(log_p_other, samples, prior_wts, n_normal, *args, samples_sel=None, pdraw_sel=None, n_draw_sel=None, predictive=False, **kwargs):
    ...
    dimensions.  Crucially, this model **assumes** that, once the Gaussian
    dimensions have been marginalized over the resulting likelihood function is
    narrow compared to the population width in the non-Gaussian dimensions, and
    therefore the marginalized term in the KDE can be treated as a
    delta-function, and the non-Gaussian population evaluated at the peak of the
    marginalized Gaussian in the KDE term (that is: it will not be evaluated at
    the *sample*, but rather at the extrapolated peak of the marginal Gaussian
    in the KDE).  If this assumption is violated, the
    :func:`conditional_normal_model` is appropriate.
